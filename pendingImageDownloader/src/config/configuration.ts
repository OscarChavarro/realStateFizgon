...
type Environment = {
  ...
  timeouts?: {
    startup?: {
      folderValidationRetryWaitMs?: number;
    };
    download?: {
...
export class Configuration {
  ...
  constructor() {
    const environmentRaw = readFileSync(join(process.cwd(), 'environment.json'), 'utf-8');
    this.environment = JSON.parse(environmentRaw) as Environment;
  ...
  get rabbitMqQueue(): string {
    return 'pending-image-urls-to-download';
  }
  ...
  get startupFolderValidationRetryWaitMs(): number {
    return Math.max(1000, this.environment.timeouts?.startup?.folderValidationRetryWaitMs ?? 3600000);
  }
