...
export class ExecuteScrapeNewPropertiesCycleUseCase {
  ...
  constructor(
    ...
    private readonly executeScrapeNewPropertiesFlowUseCase: ExecuteScrapeNewPropertiesFlowUseCase
  ) {}
  ...
  async execute(cdpHost: string, cdpPort: number): Promise<void> {
    ...
    try {
      ...
      await this.executeScrapeNewPropertiesFlowUseCase.execute(client, scrapeRunContext);
    } finally {
      await client.close();
    }
  }
