...
describe('ProcessScraperStateTransitionUseCase', () => {
  ...
  it('whenIdleHasNoPendingRequestAndSchedulerPromotes_execute_shouldReturnTrue', async () => {
    // Arrange
    const stateMachine = createStateMachineMock();
    stateMachine.getCurrentState.mockReturnValue(ScraperState.IDLE);
  ...
  it('whenIdleHasNoPendingRequestAndSchedulerDoesNotPromote_execute_shouldReturnFalse', async () => {
    // Arrange
    const stateMachine = createStateMachineMock();
    stateMachine.getCurrentState.mockReturnValue(ScraperState.IDLE);
    stateMachine.getPendingRequestsCount.mockReturnValue(0);
